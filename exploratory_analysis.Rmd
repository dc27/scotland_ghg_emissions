...
```{r}
n_children <- ghg_emissions_clean %>% 
  select(source_name) %>% 
  mutate(n_breaks = str_count(source_name, " - ")) %>% 
  distinct(n_breaks) %>% 
  pull()
...
ghg_emissions_clean %>% 
  separate(source_name, " - ", into = c(child_names))
```

```{r}
ghg_true_emissions %>% 
  distinct(source_name) %>% 
  mutate(source_name = str_count(source_name, " - ")) %>% 
  summarise(max(source_name))
```
